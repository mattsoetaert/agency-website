      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between">
        ...
        {authorized && (
          <form action="/api/sell/logout" method="post">
            <button className="text-lg text-black hover:text-black transition-colors" type="submit">
              Sign out
            </button>
          </form>
        )}
  ...
  return (
    <main className="min-h-[calc(100vh-56px)] bg-[#f5f4f0] flex items-center justify-center px-4 py-16">
      <div className="w-full max-w-sm bg-white border border-black/10 p-6 sm:p-8 shadow-[0_24px_80px_rgba(0,0,0,0.08)]">
        <p className="text-black text-base font-semibold uppercase tracking-[0.24em] mb-4">Staff Only</p>
        ...
        {error && (
          <div className="mb-4 border border-red-200 bg-red-50 px-4 py-3 text-lg text-red-700">{error}</div>
        )}
        {!isStaffCheckoutConfigured() && (
          <div className="mb-4 border border-amber-200 bg-amber-50 px-4 py-3 text-lg text-amber-800">
        ...
        <form action="/api/sell/access" method="post" className="space-y-4">
          <label className="block">
            <span className="block text-base font-medium text-black mb-2">Password</span>
            <input
              name="password"
              type="password"
              autoComplete="current-password"
              required
              className="w-full px-4 py-3 bg-[#f5f4f0] border border-black/10 text-black text-lg focus:outline-none focus:ring-1 focus:ring-black"
            />
          </label>
          <button
            type="submit"
            className="w-full px-6 py-3 bg-black hover:bg-neutral-800 text-white font-semibold text-lg transition-colors"
          >
            Unlock Checkout
          </button>
        </form>
    ...
    <main className="bg-[#f5f4f0] min-h-[calc(100vh-56px)] py-12 sm:py-16">
      <div className="max-w-5xl mx-auto px-4 sm:px-6">
        <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_340px] lg:items-start">
          <section>
            <p className="text-black text-base font-semibold uppercase tracking-[0.24em] mb-4">Subscription Checkout</p>
            ...
            <form action="/api/sell/checkout" method="post" className="bg-white border border-black/10 p-6 sm:p-8 space-y-4">
              <div className="grid gap-4 sm:grid-cols-2">
                <label className="block">
                  <span className="block text-base font-medium text-black mb-2">Client Name</span>
                  <input
                    name="clientName"
                    type="text"
                ...
                <label className="block">
                  <span className="block text-base font-medium text-black mb-2">Business Name</span>
                  <input
                    name="businessName"
                    type="text"
              ...
              <label className="block">
                <span className="block text-base font-medium text-black mb-2">Client Email</span>
                <input
                  name="clientEmail"
                  type="email"
              ...
              <label className="block">
                <span className="block text-base font-medium text-black mb-2">Phone</span>
                <input
                  name="phone"
                  type="tel"
              ...
              <label className="block">
                <span className="block text-base font-medium text-black mb-2">Internal Notes</span>
                <textarea
                  name="notes"
                  rows={4}
                  maxLength={500}
                  placeholder="Scope, launch target, or other call notes."
                  className="w-full px-4 py-3 bg-[#f5f4f0] border border-black/10 text-black placeholder-neutral-400 text-lg resize-none focus:outline-none focus:ring-1 focus:ring-black"
                />
              </label>
              ...
              <button
                ...
                className="w-full px-6 py-3 bg-black hover:bg-neutral-800 disabled:bg-neutral-300 disabled:text-black text-white font-semibold text-lg transition-colors"
              >
          ...
          <aside className="bg-white border border-black/10 p-6">
            <p className="text-black text-base font-semibold uppercase tracking-[0.24em] mb-4">Plan</p>
